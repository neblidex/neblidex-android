...
namespace NebliDex_Mobile.Droid
{
    public partial class MainService : Android.App.Service
    {
        ...
        public static void AddMyRecentTrade(int market, int ordertype, decimal price, decimal amount, string txhash, int pending)
        {
            ...
            lock (HistoricalTradeList)
            {
                //Add to historical trades, will update on the right view
                HistoricalTradeList.Insert(0, new MyTrade { Date = format_date, Pair = format_market, Type = format_type, Price = String.Format(CultureInfo.InvariantCulture, "{0:0.########}", price), Amount = String.Format(CultureInfo.InvariantCulture, "{0:0.########}", amount), TxID = txhash });
            }
        }
        ...
        public static void UpdateMyRecentTrade(string txhash, int pending)
        {
            ...
            lock (HistoricalTradeList)
            {
                //Update the historical trade
                for(int i = 0;i < HistoricalTradeList.Count; i++)
                {
                    MyTrade ht = HistoricalTradeList[i];
                    if(ht.TxID == txhash)
                    {
                    ...
                    if (utxo != null)
                    {
                        ...
                        if (total_utxo > 20 || mixed_unspent == true)
                        {
                            ...
                            if (create_tx == true)
                            {
                                ...
                                Transaction tx = CreateSignedP2PKHTx(WalletList[i].type, balance, WalletList[i].address, true, false);
                                //This should broadcast as well
                                if (tx != null)
                                {
                                    AddMyTxToDatabase(tx.GetHash().ToString(), WalletList[i].address, WalletList[i].address, balance, WalletList[i].type, 2, UTCTime());
                                }
                            }
        ...
        public static void PruneDatabases()
        {
            ...
            if (File.Exists(App_Path + "/debug.log") == true)
            {
                ...
                { //Debug log is greater than 10MB
                    lock (debugfileLock)
                    {
                        File.Delete(App_Path + "/debug.log");
                    }
                }
            }
        ...
        public static bool ExportTradeHistory(string fname)
        {
            try
            {
                ...
                new System.IO.StreamWriter(@fname, false))
                {
                    ...
                    while (statement_reader.Read())
                    {
                        ...
                        string format_market = MarketList[market].format_market;
